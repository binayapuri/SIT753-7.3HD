...
✅ COMPLETED STAGES:
• Code Checkout ✅ - Latest code retrieved from GitHub
...
• EC2 Deployment ✅ - Direct build deployment successful
• Release Tagging ✅ - Version v1.${env.BUILD_NUMBER} created
...
3. Monitor the health check logs for any issues
4. Your CI/CD pipeline is now ready for future deployments!

Congratulations on your successful DevOps pipeline deployment! 🚀
                    """.trim()
                    
                    mail(
                        to: "${NOTIFICATION_EMAIL}",
                        subject: "✅ SUCCESS: ${env.JOB_NAME} - Build #${env.BUILD_NUMBER} - Deployed to Production!",
                        body: deploymentSummary
                    )
...
🔍 FAILURE DETAILS:
The CI/CD pipeline encountered an error and could not complete successfully.
...
The CI/CD pipeline will need to be re-run after the fix.
...
Contact your DevOps team if you need assistance resolving this issue.
                    """.trim()
                    
                    mail(
                        to: "${NOTIFICATION_EMAIL}",
                        subject: "❌ FAILED: ${env.JOB_NAME} - Build #${env.BUILD_NUMBER} - Immediate Action Required",
                        body: failureDetails
                    )
